...
const PERMISSION_ACCESS = {
  trial: permissionAccessToFalse(defaultAccess, [47, 49]),
  startup: permissionAccessToFalse(defaultAccess, [14, 28, 37, 47, 49, 56, 57, 58, 60]),
  business: permissionAccessToFalse(defaultAccess, [14]),
  scale: defaultAccess,
  enterprise: defaultAccess
}

function permissionAccessToFalse (defaultAccess, accessId) {
  const accessPlan = defaultAccess.map((dataAccess) => {
...
function getPermission (permissionAcces, plan) {
  if (plan in instancePermission) {
    console.log('masuk if', plan)
    return instancePermission[plan]
  }
  console.log('tidak masuk if')
  const permissionGroupList = PERMISSIONS_GROUP_LIST.slice()
  ...
  for (let i = 0; i < lengthPermissionGroup; i++) {
    ...
    for (let j = 0; j < lengthPermissions; j++) {
      ...
      p.active = getPermissionActive(permissionAcces, p.id)
    }
  }
...
module.exports = {
  permission: {
    trial: getPermission(PERMISSION_ACCESS.trial, 'trial'),
    startup: getPermission(PERMISSION_ACCESS.startup, 'startup'),
    business: getPermission(PERMISSION_ACCESS.business, 'business'),
    scale: getPermission(PERMISSION_ACCESS.scale, 'scale'),
    enterprise: getPermission(PERMISSION_ACCESS.enterprise, 'enterprise')
  },
